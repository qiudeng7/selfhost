...
docker_setup_env() {
	...
	# 禁用旧数据库检测，因为我们的数据挂载在 /var/lib/postgresql/18/docker
...
_main() {
	...
	if [ "$1" = 'postgres' ] && ! _pg_want_help "$@"; then
		...
		if [ -z "$DATABASE_ALREADY_EXISTS" ]; then
			...
			# 数据库已存在，仍然启动临时服务器来运行初始化脚本
			cat <<-'EOM'

				PostgreSQL Database directory appears to contain a database; Running initialization scripts anyway.
